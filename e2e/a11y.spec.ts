...
test.describe("Accessibilité (a11y)", () => {
  ...
  test("TimeBar : pas de violations axe critiques", async ({
    page,
  }) => {
    await page.goto("/");
    await expect(page.getByRole("heading", { level: 1 })).toBeVisible({ timeout: 15000 });
    const timebar = page.getByTestId("map-timebar");
    await expect(timebar).toBeVisible({ timeout: 8000 });
    ...
    expect(
      critical,
      `Violations critiques axe (TimeBar) : ${JSON.stringify(critical, null, 2)}`
    ).toEqual([]);
  });
