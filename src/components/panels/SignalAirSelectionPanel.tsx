...
const SignalAirSelectionPanel: React.FC<SignalAirSelectionPanelProps> = ({
  ...
  return (
    <SidePanelShell
      ...
      onHidden={onHidden}
      onClose={onClose}
      closeLabel={t("panels.closeAndDisableSignalAir")}
      width="compact"
      testId="signalair-selection-panel"
      title={t("panels.signalAirSelection.title")}
      subtitle={pollutantLabel}
      badge={
        <span
          className="flex shrink-0 items-center rounded-[var(--r-xs)] border border-[#13A0DB]/30 bg-[#13A0DB]/10 p-1"
          title={t("panels.signalAirSelection.reopenButtonTooltip")}
        >
          <span
            className="block h-5 w-5"
            aria-hidden="true"
            dangerouslySetInnerHTML={{ __html: SIGNALAIR_MARK_SVG }}
          />
        </span>
      }
    >
          ...
          {markerPreview.map((type) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-start space-x-3 rounded-[var(--r-md)] border p-3 transition-all ${
                  isSelected
                    ? "border-blue-300 bg-blue-50"
                    : "border-[rgb(16_32_56_/_0.09)] bg-white hover:bg-black/5"
                }`}
              >
                <div
      ...
      <div className="border border-[rgb(16_32_56_/_0.09)] rounded-[var(--r-md)] p-3 sm:p-4 space-y-2">
        <button
          ...
          className={`w-full py-3 px-4 rounded-[var(--r-md)] font-medium transition-all ${
            isLoadDisabled
