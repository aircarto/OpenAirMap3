...
const StationSidePanel: React.FC<StationSidePanelProps> = ({
  ...
  // Il est volontairement distinct de `state.chartControls.timeStep` (le pas de
  // temps sélectionné dans l'UI) : pendant un rechargement, le graphique doit
  // continuer d'afficher un couple cohérent (anciennes données + ancien pas de
  // temps) et basculer d'un seul coup quand les nouvelles données arrivent.
  // Sinon les anciennes données sont re-échantillonnées sur la nouvelle grille
  // et la courbe se réduit à des points isolés entourés de null.
  ...
  // Rechargements suivants : voile par-dessus le graphique déjà affiché, qui
  // reste monté.
  const isInitialChartLoading =
    state.loading && Object.keys(state.historicalData).length === 0;
  // Empêcher le déclenchement d'un second chargement pendant qu'un autre est en
  // cours : les contrôles restent visibles sous le voile, donc cliquables.
  ...
  // Le montage, l'animation de sortie et son portail appartiennent désormais à
  // SidePanelShell : il ne reste ici que la garde sur les données du panneau.
  if (!selectedStation) {
    return null;
  }
